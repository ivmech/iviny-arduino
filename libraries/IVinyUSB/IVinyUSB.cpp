 ...
 * Modification for the Iviny by Caner Durmusoglu, Ivmech Mechatronics Co.
 * VID/PID changed to pair owned by Ivmech Mechatronics Co., code modified to use pinchange int for attiny85
...
#if F_CPU != 16500000L
  #error "You must use IViny board to use USB libraries"
#endif
...
IVinyUSBDevice::IVinyUSBDevice(ring_buffer *rx_buffer,
				 ring_buffer *tx_buffer) {
  _rx_buffer = rx_buffer;
  _tx_buffer = tx_buffer;
}

void IVinyUSBDevice::begin() {
    cli();
...
void IVinyUSBDevice::update() {
  refresh();
}


void IVinyUSBDevice::refresh() {
  usbPoll();
}

// wait a specified number of milliseconds (roughly), refreshing in the background
void IVinyUSBDevice::delay(long milli) {
...
size_t IVinyUSBDevice::write(byte c) {
  /*
   */
  return store_char(c, _tx_buffer);
}

void IVinyUSBDevice::setMaxByte(int b) {
    if (b > MAX_BYTE) b = 40;
    max_tx_byte = b;
}
...
usbMsgLen_t usbFunctionSetup(uchar data[8])
{
  usbRequest_t    *rq = (usbRequest_t*)((void *)data);
    ...
    if((rq->bmRequestType & USBRQ_TYPE_MASK) == USBRQ_TYPE_CLASS){    /* HID class request */
	    ...
	    static uchar dataBuffer[MAX_BYTE];  /* buffer must stay valid when usbFunctionSetup returns */
	    if (tx_available()) {
          int i;
          for (i=0; i < max_tx_byte; i++) {
	        dataBuffer[i] = tx_read();
          }
	      usbMsgPtr = dataBuffer; /* tell the driver which data to return */
	      return max_tx_byte; /* tell the driver to send 1 byte */
	    } else {
...
IVinyUSBDevice IVinyUSB = IVinyUSBDevice(&rx_buffer, &tx_buffer);
